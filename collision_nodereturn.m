function [rn,links,connectivity,linksinconnect,fseg,stackforcevec,node1_index,node2_index]=collision_nodereturn(rn,links,connectivity,linksinconnect,fseg,mindist,MU,NU,a,Ec,mobility,appliedstress,dopartials,stackforcevec,rntol,doSFT,SFT_plane,doinclusion,inclusion_pos_rad,doshielding,SurfacePlane,dobox,boxD)
...
% use rn_belong to tell which loops have collided
rn_belong = rn(:,end);
rn = rn(:,1:end-1);
    ...
    while j<=size(links,1)
        ...
        if (n1s1~=n1s2)&(n1s1~=n2s2)&(n2s1~=n1s2)&(n2s1~=n2s2)
            ...
            if collision_condition_is_met
                ...
                if close_to_n1s1
                    mergenode1=n1s1;
                    node1_index = rn_belong(mergenode1,1);
                elseif close_to_n2s1
                    mergenode1=n2s1;
                    node1_index = rn_belong(mergenode1,1);
                else
                    spnode=n1s1;
                    node1_index = rn_belong(n1s1,1);
...
%                     end
                ...
                if close_to_n1s2 
                    mergenode2=n1s2;
                    node2_index = rn_belong(mergenode2,1);
                elseif close_to_n2s2
                    mergenode2=n2s2;
                    node2_index = rn_belong(mergenode2,1);
                else
                    spnode=n1s2;
                    node2_index = rn_belong(n1s2,1);
...
%                     end
                ...
                if mergednodeid>0
                    for k=1:connectivity(mergednodeid,1)
                        ...
                        [rn(othernode,4:6),fntmp]=feval(mobility,fseg,rn,links,connectivity,othernode,clist,mobility,dopartials,stackforcevec,rntol,doSFT,SFT_plane,doinclusion,inclusion_pos_rad);
                    end
                    numbcon=connectivity(mergednodeid,1);
                    conlist=[numbcon linspace(1,numbcon,numbcon)];
                    [rn(mergednodeid,4:6),fntmp]=feval(mobility,fseg,rn,links,connectivity,mergednodeid,conlist,mobility,dopartials,stackforcevec,rntol,doSFT,SFT_plane,doinclusion,inclusion_pos_rad);
                end
...
if exist('node1_index','var') == 0
    node1_index = NaN;
end
 if exist('node2_index','var') == 0
    node2_index = NaN;
end
