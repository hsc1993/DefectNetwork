function [vn,fn] = mobfccPFZ(fseg,rn,links,connectivity,nodelist,conlist,mobility,dopartials,stackforcevec,rntol,doSFT,SFT_plane,doinclusion,inclusion_pos_rad)
...
for n=1:L1
    ...
    if(doinclusion)
        for k=1:size(inclusion_pos_rad,1)
            if(norm(rn(n0,1:3)-inclusion_pos_rad(k,1:3))<inclusion_pos_rad(k,4))
                Bscrew=inclusion_pos_rad(k,8);
                Bedge=inclusion_pos_rad(k,9);
                Bline=1.0e-2*min(Bscrew,Bedge);
            else
                Bscrew=12.84e-25;
                Bedge=3.0214e-25;
    ...
    posi = rn(n,1:3);
    if posi(1) < -2000
        Bscrew=0.4*7.5e-25;%IF THE UNIT LENGTH IS THE LATTICE PARAMETER (Enrique Sep 2005)
        Bedge=0.4*3.9e-25;
        Bclimb=1e-12;%Almost no climb at all
        Bline=1.0e-2*min(Bscrew,Bedge);
     elseif posi(1) >= -2000
...
%          Bline=1.0e-4*min(Bscrew,Bedge);     
    ...
    for i=1:numNbrs
        ...
        if L>0.0
            ...
            if ~((abs(dot(nplane,[1 1 1]/norm([1 1 1])))>0.99)|(abs(dot(nplane,[-1 1 1]/norm([-1 1 1])))>0.99)|(abs(dot(nplane,[1 -1 1]/norm([1 -1 1])))>0.99)|(abs(dot(nplane,[1 1 -1]/norm([1 1 -1])))>0.99))
    ...
    if rn(n,4)==3
        if numNbrs == 2
            phi = atan2(norm(cross(linelist(1,:),linelist(2,:))), dot(linelist(1,:),linelist(2,:)));
        else
            disp('Error, precipitate node is a junction node.... What to do?'); 
        end
%        disp(sprintf('phi=%f, line1=%f %f %f line2=%f %f %f rn=%f',phi,linelist(1,:),linelist(2,:),rn(n,:)));
        if abs(phi) < phic
            % Do not pin if chritical angle is reached
            disp(sprintf('chritical angel phi=%f reached, shearing precipitate...  %f, %f %f %f\n',phi,vn(n,:))); 
            %rn(n,4)=3;
        elseif abs(phi) >= phic
            % Pin precipitate
            disp(sprintf('dislocation pinned at precipitate, phi=%f... %f, %f %f %f\n',phi,size(fn(n,:),2),rn(n,:))); 
            
%         Bscrew=Bscrew*1.0e22;
%         Bedge=Bedge*1.0e22;
%         Beclimb=Beclimb*1.0e22;
%         Bline=1.0e-4*min(Bscrew,Bedge); 
