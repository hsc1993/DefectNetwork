function [vn,fn] = mobfcc0(fseg,rn,links,connectivity,nodelist,conlist,mobility,dopartials,stackforcevec)
%mobility law function (model: FCC0)

SFT=8; %Introduction of SFT volume
SFTtol=0.001;
%Drag (Mobility) parameters
Bscrew=15.68e-20;%IF THE UNIT LENGTH IS THE LATTICE PARAMETER (Enrique Sep 2005)
Bedge=15.68e-20;
Bclimb=15.68e-7;%Almost no climb at all
Franknorm=(1/3)*norm([1 1 1 ]);
    ...
    for i=1:numNbrs
        ...
        if L>0.0
            ...
            if ~((abs(dot(nplane,[1 1 1]/norm([1 1 1])))>0.95)|(abs(dot(nplane,[-1 1 1]/norm([-1 1 1])))>0.95)|(abs(dot(nplane,[1 -1 1]/norm([1 -1 1])))>0.95)|(abs(dot(nplane,[1 1 -1]/norm([1 1 -1])))>0.95))|...%The plane has to be a {111} plane % this is a second condition that checks for good glide planes
                ...
                if (abs(norm(burgv)-Franknorm)<eps)
                    Bclimb=-15.68e-19;
                end
