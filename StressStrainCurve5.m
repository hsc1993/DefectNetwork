...
for radius=5%3:2:13
    for i=5%1:length(le)
        ...
        filename = sprintf('Stress_Strain_curves/plotforpaper/Stress_Strain_curve_r%02.0f_length%03.0f_100_ppt_screw.txt',radius,le(i));
        videoname = sprintf('Stress_Strain_curves/plotforpaper/dislocation_r%02.0f_length%03.0f_100_ppt_screw.avi',radius,le(i));
        ...
        plotfilename=sprintf('Stress_Strain_curves/plotforpaper/dislocation_at_time%i',totaltime);
...
%         end  
            ...
            while(1)
                ...
                if(conv==1)
                    ...
                    plotnodes(rn,links,plim,doinclusion,inclusion_pos_rad,SurfacePlane); %view(viewangle); xlim([-plim/zoom plim/zoom]); ylim([-plim/zoom plim/zoom]); zlim([-plim/zoom plim/zoom]);
                    plotfilename=sprintf('Stress_Strain_curves/plotforpaper/dislocation_at_time%i.png',curstep);
                    print(plotfilename,'-dpng');
                    frame = getframe;
                    %movegui(frame, 'onscreen');
                    %hold all;
                    %datetick;
                    %drawnow;
                    writeVideo(vidObj,frame); 
