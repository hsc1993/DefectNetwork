function [vn,fn] = mobbcc0b(fseg,rn,links,connectivity,nodelist,conlist)
%mobility law function (model: BCC0b)
    ...
    for i=1:numNbrs
        ...
        if L>0.0
            ...
            if abs(burgv(1)*burgv(2)*burgv(3))<eps
                ...
                burgdir=burgv./burgmag;
                Ls=L*sqrt(cth2);
                Le=L*sqrt(1-cth2);
                Btotal=Btotal+(2.0*Ls).*((Bscrew).*eye(3)+(Bline-Bscrew).*(burgdir'*burgdir));           % build the drag matrix assuming that the dislocation is screw type
                if ( 1 - cth2 ) >eps
                    mdir=burgdir;
                    ndir=cross(mdir,linedir)./sqrt((1-cth2));                                            % correct the drag matrix for dislocations that are not screw type
                    pdir=cross(ndir,mdir);
                    Btotal=Btotal+(2.0*Le).*( Bedge .*(mdir'*mdir) + Beclimb.*(ndir'*ndir) + Bline.*(pdir'*pdir) );
                end
            end
        end
    end
...
%        L=norm(rt);
%        linedir=rt./L;
%        vn(n,:)=((eye(3)-(10d0-1.0d2/Bline).*linedir'*linedir)*vn(n,:)')';
%    end
end